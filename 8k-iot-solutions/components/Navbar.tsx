...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 flex justify-center pointer-events-none font-sans">
      ...
      <nav 
        className={`pointer-events-auto relative flex items-center justify-between transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]
          ${isScrolled 
            ? 'mt-4 md:mt-6 w-[98%] max-w-7xl rounded-2xl bg-white/95 backdrop-blur-md shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-white/50 py-2 px-4 sm:px-6' 
            : 'mt-0 w-full max-w-full rounded-none bg-white/95 backdrop-blur-sm shadow-sm border-b border-gray-100 py-3 px-4 sm:px-8 lg:px-12'
          }
        `}
      >
        ...
        <div className="md:hidden flex flex-1 items-center justify-end">
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 text-brand-900 focus:outline-none">
