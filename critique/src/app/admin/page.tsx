...
// --- TYPES ---
// Added 'updates' to the tabs
type Tab = 'analytics' | 'content' | 'users' | 'reports' | 'updates';

// --- GRAPH COMPONENTS (Kept the same) ---
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    let topTags = Object.entries(tagCounts).sort(([, a], [, b]) => b - a).map(([label, value]) => ({ label, value }));
  ...
  const toggleLock = async (id: string, current: boolean) => {
    ...
    const { error, count } = await supabase.from('submissions').update({ is_locked: !current }).eq('id', id).select();
    if (error || count === 0) {
  ...
  const toggleHide = async (id: string, current: boolean) => {
    ...
    const { error, count } = await supabase.from('submissions').update({ is_hidden: !current }).eq('id', id).select();
    if (error || count === 0) {
      alert("Failed to hide thread.");
      setSubmissions(prev => prev.map(s => s.id === id ? { ...s, is_hidden: current } : s));
    }
  };
        ...
        <div className="w-56 border-r border-border bg-panel flex flex-col overflow-y-auto">
          <div className="p-4 flex flex-col gap-1">
            ...
            {[
              ...
              { id: 'updates', label: 'System Blast', count: 0 }, // NEW TAB
            ].map((tab) => (
              <button
                key={tab.id}
        ...
        <div className="flex-1 p-8 bg-background overflow-y-auto">
          ...
          {activeTab === 'analytics' && stats && (
            <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
               {/* Same Analytics UI as before... */}
               <div className="flex justify-between items-end"><div><h2 className="text-3xl font-black uppercase italic tracking-tighter">Analytics Overview</h2><p className="text-sm text-gray-500 font-medium">Real-time platform metrics.</p></div></div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-panel border border-border p-8">
                  <h3 className="text-xs font-black uppercase tracking-widest text-gray-500 mb-6">Submission Activity</h3>
                  <BarChart data={stats.timelineData} color="bg-ytRed" />
                </div>
                <div className="bg-panel border border-border p-8">
                  <h3 className="text-xs font-black uppercase tracking-widest text-gray-500 mb-6">Top Tags</h3>
                  ...
                  <tbody>
                    {submissions.map((sub) => (
                      <tr key={sub.id} className={`border-b border-border transition-colors ${sub.is_hidden ? 'bg-gray-900/30 opacity-60' : 'hover:bg-background/50'} ${sub.is_locked ? 'border-l-4 border-l-red-600' : ''}`}>
                        <td className="p-4"><span className="font-bold text-foreground block">{sub.channel_name}</span><a href={`/channel/${sub.id}`} target="_blank" className="text-[10px] text-gray-500 hover:text-ytRed">View Thread →</a></td>
                        ...
                        <td className="p-4 flex justify-end gap-2">
                          <button onClick={() => toggleLock(sub.id, sub.is_locked)} className="min-w-[80px] px-3 py-1.5 text-[10px] font-bold uppercase border bg-background border-border text-gray-400 hover:border-red-500 hover:text-red-500">{sub.is_locked ? 'Unlock' : 'Lock'}</button>
                          <button onClick={() => toggleHide(sub.id, sub.is_hidden)} className="min-w-[80px] px-3 py-1.5 text-[10px] font-bold uppercase border bg-background border-border text-gray-400 hover:border-foreground hover:text-foreground">{sub.is_hidden ? 'Unhide' : 'Hide'}</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                 ...
                 {users.map((user) => (
                   <div key={user.id} className="flex justify-between items-center p-4 border border-border bg-panel shadow-sm hover:border-gray-500 transition-colors">
                      <div className="flex items-center gap-4">
                         <div className="w-10 h-10 bg-gray-700 rounded-full overflow-hidden"><img src={user.avatar_url || "https://ui-avatars.com/api/?background=random"} className="w-full h-full object-cover" /></div>
                         <div><p className="font-bold text-sm flex gap-2 items-center">{user.full_name || "Anonymous"} {user.role === 'admin' && <span className="bg-ytRed text-white text-[9px] px-1.5 py-0.5 rounded">ADMIN</span>}</p><p className="text-[10px] text-gray-500 font-mono">{user.email}</p></div>
                      </div>
                      {user.role !== 'admin' && <button onClick={() => toggleBan(user.id, user.is_banned)} className={`text-[10px] font-black uppercase px-4 py-2 border ${user.is_banned ? 'border-green-500 text-green-500' : 'border-red-500 text-red-500'}`}>{user.is_banned ? 'Unban' : 'Ban'}</button>}
                   </div>
                 ))}
               </div>
            </div>
